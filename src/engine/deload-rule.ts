...
export function computeWeekSignals(weekSessions: Session[]): WeekSignals {
  ...
  for (const s of weekSessions) {
    ...
    // C3 week 4 (and any deload) has no Volume block at all (volumeDifficulty
    // null that session) — nothing to be "incomplete", so S2 never evaluates it.
    if (s.volumeDifficulty != null && s.volDone === false) {
      types.add('S2')
      occurrences++
      hit = true
    }
